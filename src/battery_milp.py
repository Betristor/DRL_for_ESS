...
class BatteryMILP:
    ...
    def optimise(
        ...
        model.soc = Var(
            model.T,
            bounds=(
                0.2 * model.cap * (remaining_capacity / 100),
                model.cap * (remaining_capacity / 100),
            ),
            initialize=0.2 * model.cap * (remaining_capacity / 100),
        )
        ...
        def update_soc(model, t):
            if t == 0:
                return model.soc[t] == intial_soc - ((model.energy_out[t])) + (
                    ((model.energy_in[t]))
                )
            else:
                return model.soc[t] == model.soc[t - 1] - ((model.energy_out[t])) + (
                    ((model.energy_in[t]))
                )
        ...
        def timeseries_profit(model, t):
            current_profit = (
                model.energy_out[t] * model.price[t] * discharge_efficiency
            ) - (model.energy_in[t] * model.price[t] / charging_efficiency)
        ...
        def cycle_rate_per_ts(model, t):
            ...
            if t == 0:
                ...
                return (
                    model.cumlative_cycle_rate[t]
                    == model.cumlative_cycle_rate[t - 1] + ts_cycle
                )
        ...
        def cumlative_profit(model, t):
            if t == 0:
                return model.cumulative_profit[t] == (
                    model.energy_out[t] * model.price[t] * discharge_efficiency
                ) - (model.energy_in[t] * model.price[t] / charging_efficiency)
            else:
                return model.cumulative_profit[t] == model.cumulative_profit[t - 1] + (
                    (model.energy_out[t] * model.price[t] * discharge_efficiency)
                    - (model.energy_in[t] * model.price[t] / charging_efficiency)
                )
        ...
        if previous_ep_power != 0:
            alpha_degradation = (
                (((self.previous_cap - remaining_capacity) / 100) * self.cap)
                / previous_ep_power
            ) * self.batt_cost
        else:
            alpha_degradation = 0
        ...
        degrade_cost = [
            alpha_degradation * (model.energy_out[t] + model.energy_in[t])
            for t in model.T
        ]

        # define profit
        export_revenue = [
            price_df.iloc[t, -1] * model.energy_out[t] * discharge_efficiency
            for t in model.T
        ]
        import_cost = [
            price_df.iloc[t, -1] * model.energy_in[t] / charging_efficiency
            for t in model.T
        ]
        profit_ts = (
            np.array(export_revenue) - np.array(import_cost) - np.array(degrade_cost)
        )
