...
class BatteryEfficiency:
    ...
    def ss_circuit_model(self, soc):
        v_oc = (
            (self._params["a_0"] * np.exp(-self._params["a_1"] * soc))
            + self._params["a_2"]
            + (self._params["a_3"] * soc)
            - (self._params["a_4"] * soc**2)
            + (self._params["a_5"] * soc**3)
        ) * self._cellnum
        r_s = (
            (self._params["b_0"] * np.exp(-self._params["b_1"] * soc))
            + self._params["b_2"]
            + (self._params["b_3"] * soc)
            - (self._params["b_4"] * soc**2)
            + (self._params["b_5"] * soc**3)
        ) * self._cellnum
        r_st = (
            self._params["c_0"] * np.exp(-self._params["c_1"] * soc)
            + self._params["c_2"]
        ) * self._cellnum
        r_tl = (
            self._params["e_0"] * np.exp(-self._params["e_1"] * soc)
            + self._params["e_2"]
        ) * self._cellnum
        r_tot = r_s + r_st + r_tl
        return v_oc, r_tot
